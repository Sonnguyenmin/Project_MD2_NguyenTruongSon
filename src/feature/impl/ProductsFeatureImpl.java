...
public class ProductsFeatureImpl implements IProduct {
    ...
    public void save(Products element) {
        if (findById(element.getProduct_id()) == null) {
            productList.add(element);
        }
        else {
            productList.set(productList.indexOf(findById(element.getProduct_id())), element);
        }
    ...
    public Integer getNewId() {
        int maxId = 0;
        for (Products product : productList) {
            if (product.getProduct_id() > maxId) {
                maxId = product.getProduct_id();
            }
        }
        return maxId + 1;
    }

...
public class AddressUserManagement {
    ...
    private void displayAllAddress() {
        ...
        if (addressFeature.getAll().stream().filter(item-> item.getUserId().getUserId() == user.getUserId()).count() <= 0) {
    ...
    private void deleteAddressById() {
        ...
        for(Address address :addressFeature.getAll()){
            if (address.getAddressId() == idAddress && address.getUserId().getUserId() == user.getUserId()){

...
public class WithListUserManagement {
        ...
        do {
            ...
            switch (choice) {
                ...
                case 4:
                    ...
                    break;
                default:
                    System.err.println(RED + "Vui lòng nhập lại từ 1 -> 5" + RESET);
            }
        } while (isExist);
    }
    ...
    public void showAllWishListProduct() {
        ...
        if (user == null || wishListFeature.getAll().stream().filter(item ->item.getUserId().getUserId() == user.getUserId()).count() <= 0) {
        ...
        for (WishList wishList : wishListFeature.getAll()) {
            if (wishList.getUserId().getUserId() == user.getUserId()) {
    ...
    public void addWishListProduct() {
        ...
        do {
            ...
            if (selectedProduct == null) {
                ...
                if (wishListFeature.getAll().stream().anyMatch(item -> item.getUserId().getUserId() == user.getUserId()
    ...
    public void deleteWishListProduct() {
        ...
        for(WishList wishList :wishListFeature.getAll()){
            if (wishList.getWishListId() == idWishList && wishList.getUserId().getUserId() == user.getUserId()){
